...
async def parse_match(session: aiohttp.ClientSession, event_name: str, match_url: str) -> tuple[list,list]:
    ...
    # Derive stage and match_type heuristically from match_name
    stage = ''
    match_type = ''
    ...
    if len(parts) >= 2:
        match_type = parts[-1]
        stage_token = parts[-2]
        # Try to extract a canonical stage keyword from the token
    ...
    for game_div in soup.select('div.vm-stats-game'):
        ...
        # Remove any time-like or numeric suffixes (e.g., 58:09, 1:06:22, 1:)
        ...
        # Normalize to known map names if present
        KNOWN_MAPS = ['Ascent','Bind','Breeze','Fracture','Haven','Icebox','Lotus','Pearl','Split','Sunset','Abyss']
        ...
        for ev_url in VCT_2025_EVENTS:
            ...
            try:
                ...
                for scores_sets, overview in results:
                    # Skip matches with no map data or stats
                    if len(scores_sets[1]) == 0 and len(scores_sets[2]) == 0 and len(overview) == 0:
                        continue
